...
architecture rtl of riscv_core_memless is
  ...
  signal fi_state_strobe_a : std_logic;
  signal fi_state_strobe_b : std_logic;
  signal fi_state_strobe_c : std_logic;
  ...
  signal fi_pc_strobe_a   : std_logic;
  signal fi_pc_strobe_b   : std_logic;
  signal fi_pc_strobe_c   : std_logic;
...
begin
  ...
  --------------------------------------------------------------------
  fi_pc_strobe_a <= fi_pc_strobe_i when fi_pc_target_i = "00" else '0';
  fi_pc_strobe_b <= fi_pc_strobe_i when fi_pc_target_i = "01" else '0';
  fi_pc_strobe_c <= fi_pc_strobe_i when fi_pc_target_i = "10" else '0';

  fi_pc_a : entity work.fault_injector
    generic map(WIDTH => 32, G_ENABLE => G_FAULT_INJECT)
    port map(data_i => pc_a, mask_i => fi_pc_mask_i, strobe_i => fi_pc_strobe_a, data_o => pc_vote_a);

  fi_pc_b : entity work.fault_injector
    generic map(WIDTH => 32, G_ENABLE => G_FAULT_INJECT)
    port map(data_i => pc_b, mask_i => fi_pc_mask_i, strobe_i => fi_pc_strobe_b, data_o => pc_vote_b);

  fi_pc_c : entity work.fault_injector
    generic map(WIDTH => 32, G_ENABLE => G_FAULT_INJECT)
    port map(data_i => pc_c, mask_i => fi_pc_mask_i, strobe_i => fi_pc_strobe_c, data_o => pc_vote_c);
  ...
  fi_state_strobe_a <= fi_state_strobe_i when fi_state_target_i = "00" else '0';
  fi_state_strobe_b <= fi_state_strobe_i when fi_state_target_i = "01" else '0';
  fi_state_strobe_c <= fi_state_strobe_i when fi_state_target_i = "10" else '0';

  fi_state_a : entity work.fault_injector
    generic map(WIDTH => 4, G_ENABLE => G_FAULT_INJECT)
    port map(data_i => state_enc_a, mask_i => fi_state_mask_i, strobe_i => fi_state_strobe_a, data_o => state_vote_slv_a);

  fi_state_b : entity work.fault_injector
    generic map(WIDTH => 4, G_ENABLE => G_FAULT_INJECT)
    port map(data_i => state_enc_b, mask_i => fi_state_mask_i, strobe_i => fi_state_strobe_b, data_o => state_vote_slv_b);

  fi_state_c : entity work.fault_injector
    generic map(WIDTH => 4, G_ENABLE => G_FAULT_INJECT)
    port map(data_i => state_enc_c, mask_i => fi_state_mask_i, strobe_i => fi_state_strobe_c, data_o => state_vote_slv_c);
  ...
  --------------------------------------------------------------------
  process(dmem_rdata, cur_load_size, cur_load_sign, byte_offset)
  ...
  begin
    ...
    case cur_load_size is
      when "00" =>
        case byte_offset is
          when "00"   => b := dmem_rdata(7 downto 0);
          when "01"   => b := dmem_rdata(15 downto 8);
          when "10"   => b := dmem_rdata(23 downto 16);
          when others => b := dmem_rdata(31 downto 24);
        end case;
      ...
      when "01" =>
        if byte_offset(1) = '0' then
          h := dmem_rdata(15 downto 0);
        else
          h := dmem_rdata(31 downto 16);
        end if;
