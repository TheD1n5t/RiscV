...
architecture rtl of csr_file is
  ...
  signal dmem_ecc_single_error_d : std_logic := '0';
  signal dmem_ecc_double_error_d : std_logic := '0';
  ---------------------------------------------------------
  -- Majority voters
  ---------------------------------------------------------
  ...
  begin
    if rising_edge(clk) then
      ...
      if rst = '1' then
        ...
        wdog_count       <= (others => '0');
        watchdog_reset_r <= '0';
        dmem_ecc_single_error_d <= '0';
        dmem_ecc_double_error_d <= '0';
        ...
        if dmem_ecc_single_error_i = '1' and dmem_ecc_single_error_d = '0' then
          if dmem_ecc_single_cnt /= x"FFFFFFFF" then
            dmem_ecc_single_cnt <= dmem_ecc_single_cnt + 1;
            report "CSR: dmem single counter incremented" severity note;
          end if;
        end if;
        
        if dmem_ecc_double_error_i = '1' and dmem_ecc_double_error_d = '0' then
          if dmem_ecc_double_cnt /= x"FFFFFFFF" then
            dmem_ecc_double_cnt <= dmem_ecc_double_cnt + 1;
          end if;
        end if;
        
        dmem_ecc_single_error_d <= dmem_ecc_single_error_i;
        dmem_ecc_double_error_d <= dmem_ecc_double_error_i;
